...
ic15_root_dir = '/nas-02/hdd/chenshuanghao/sig_extraction_generation/sig_detect_dasetset_violence_new/train/'
# ic15_root_dir = '/nas-02/hdd/chenshuanghao/sig_detection_open/train/'
ic15_train_data_dir = ic15_root_dir + 'train_images/'
ic15_train_gt_dir = ic15_root_dir + 'train_gts/'

ic15_root_dir_test = '/nas-02/hdd/chenshuanghao/sig_detection_open/test/'
# ic15_root_dir_test = '/nas-02/hdd/chenshuanghao/sig_extraction_generation/sig_detect_dasetset_violence/test/'
ic15_test_data_dir = ic15_root_dir_test + 'test_images/'
ic15_test_gt_dir = ic15_root_dir_test + 'test_gts/'
...
def get_ann(img, gt_path):
    ...
    for line in lines:
        ...
        if len(gt)==8:
            ...
            print('the length of location is not equal 8')
            exit()  
        if word == '#':
            words.append('###')
        else:
...
def random_scale(img, short_size=736):
    ...
    aspect = np.random.choice(np.array([0.9, 1.0, 1.1]))
...
class PSENET_IC15(data.Dataset):
    def __init__(self,
        ...
        # temp = []
        # for img_path, gt_path in zip(self.img_paths, self.gt_paths):
        #     temp.append([img_path, gt_path])
        # temp = random.sample(temp, 30000) 
        # temp = np.array(temp)
        # self.img_paths = temp[0:, 0]
        # self.gt_paths = temp[0:, 1]

        # print('the length :{}'.format(len(self.img_paths)))
    ...
    def prepare_train_data(self, index):
        ...
        if self.is_transform:
            # 短边resize,随机尺度，保持一定宽高比 
            img = random_scale(img, self.short_size)
        temp_test = Image.fromarray(img)
        temp_test.save('img.png')
        ...
        # temp_test = np.uint8(gt_kernels[0]*255)
        # temp_test = Image.fromarray(temp_test)
        # temp_test.save('kernels_0.png')
        ...
        # temp_test = transforms.ToPILImage()(gt_kernels[5].int())
        # arr = np.array(temp_test)
        # temp_test = Image.fromarray(np.uint8(arr*255))
        # temp_test.save('test.png')
